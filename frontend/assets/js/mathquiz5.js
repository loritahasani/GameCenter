    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (level === 1) { // Easy
            ...
            if (Math.random() > 0.5) {
                text = `Sa është ${a} × ${b}?`;
                correct = a * b;
                wrong = correct + (Math.random() > 0.5 ? 8 : -8);
            } else {
                text = `Sa është ${a * b} ÷ ${a}?`;
                correct = b;
            ...
            if (Math.random() > 0.5) {
                ...
                const apples = Math.floor(Math.random() * 10) + 2;
                const friends = Math.floor(Math.random() * 4) + 2;
                text = `Nëse ke ${apples} mollë dhe i ndan me ${friends} shokë, sa mollë merr secili?`;
                correct = Math.floor(apples / friends);
                wrong = correct + 1;
            }
        }
    ...
    setTimeout(() => {
        ...
        if (currentQuestion < questions.length) {
            ...
            if (currentLevel < 3) {
                ...
                sendScoreToBackend('mathquiz5', totalScore);
                setTimeout(() => {
